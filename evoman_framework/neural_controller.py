...
class NeuralController(Controller):
    ...
    def set(self, genome: list, _=None) -> None:
        ...
        w1_end = self.w1_matrix.size
        w2_end = self.w1_matrix.size + self.w2_matrix.size
        bias1_end = w2_end + self.bias1.size

        self.w1_matrix = genome[:w1_end].reshape(self.w1_matrix.shape)
        self.w2_matrix = genome[w1_end:w2_end].reshape(self.w2_matrix.shape)
        self.bias1 = genome[w2_end:bias1_end].reshape(self.bias1.shape)
        self.bias2 = genome[bias1_end:].reshape(self.bias2.shape)
