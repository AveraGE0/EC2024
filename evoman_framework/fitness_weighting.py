...
class FitnessChangeWeighting(FitnessWeighting):
    ...
    def update(self, metrics: dict[str, np.ndarray]) -> None:
        ...
        Args:
            metrics: dict[str, np.ndarray]: Metrics to update the weighter (fitness means are used).
        """
...
class FitnessProportionalWeighting(FitnessWeighting):
    ...
    def update(self, metrics: dict[str, np.ndarray]) -> None:
        ...
        # current_weight = 1 - ((fitness_avg - fitness.min()) / (fitness.max() - fitness.min()))

        self.weights = (1-self.step_size) * self.weights + self.step_size * current_weight
        self.weights = np.clip(self.weights, self.min_weight, self.max_weight)
...
class DefeatedProportionalWeighting(FitnessWeighting):
    ...
    def update(self, metrics: dict[str, np.ndarray]) -> None:
        ...
        proportional_change = min_defeated / (defeated + 1e-7)
...
class MeanWeighting:
    ...
    def get_weighted_fitness(self, fitness_avg: np.ndarray) -> float:
        """Method to get the mean of the finesses."""
        return fitness_avg.mean()
