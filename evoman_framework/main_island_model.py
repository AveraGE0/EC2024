...
def evaluate_individual(individual):
    # Evaluate the individual against all specified enemies
...
def migrate(islands, migration_rate):
    ...
    for i in range(len(islands)):
        ...
        migrants, migrant_fitnesses = select_migrants(source_island['population'], source_island['fitness'], num_migrants)
        # Replace worst individuals in target island
        replace_individuals(target_island['population'], target_island['fitness'], migrants, migrant_fitnesses)
...
def select_migrants(population, fitness_scores, num_migrants):
    ...
    sorted_paired = sorted(paired, key=lambda x: x[1] if x[1] is not None else float('-inf'), reverse=True)
...
def replace_individuals(population, fitness_scores, migrants, migrant_fitnesses):
    ...
    sorted_paired = sorted(paired, key=lambda x: x[1] if x[1] is not None else float('inf'))
...
def log_statistics(islands, generation, filename):
        ...
        with open(filepath, 'w') as f:
            ...
            for idx in range(len(islands)):
                header += (f'\tIsland_{idx}_Mean_Fitness\tIsland_{idx}_Max_Fitness'
                           ...
                           f'\tIsland_{idx}_Best_Individual_Weights')
            f.write(header + '\n')
    ...
    for idx, island in enumerate(islands):
        ...
        # Get the best individual for each island
        best_idx = np.argmax(island['fitness'])
        best_individual = island['population'][best_idx]

        # Convert the weights to a string
        weights_str = ','.join(map(str, best_individual))
        # Append the weights to the data_line
        data_line += f'\t{weights_str}'
    ...
    for idx, island in enumerate(islands):
        ...
        data_line += f'\t{mean_fitness:.4f}\t{max_fitness:.4f}\t{min_fitness:.4f}\t{std_fitness:.4f}'

        # Optionally, print the statistics to the console
        print(
            f"Generation {generation}, Island {idx}: Mean Fitness = {mean_fitness:.4f}, Max Fitness = {max_fitness:.4f}, Min Fitness = {min_fitness:.4f}, Std Fitness = {std_fitness:.4f}")

        # Save the best individual for each island
        best_idx = np.argmax(island['fitness'])
        best_individual = island['population'][best_idx]
        np.save(os.path.join(experiment_name, f'island_{idx}_generation_{generation}_best.npy'), best_individual)
...
# After evolution, evaluate best individuals
best_individuals = []
for island in islands:
    best_idx = np.argmax(island['fitness'])
